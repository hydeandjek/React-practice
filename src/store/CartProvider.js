...
const cartReducer = (state, action) => {
  if (action.type === 'ADD') {
    ...
    // 기존 장바구니에 등록된 메뉴인지 아닌지에 따라 처리륻 다르게 해야 할 것 같다.
    const index = state.items.findIndex((item) => item.id === newCartItem.id);
    // 기존 아이템
    const existingItems = [...state.items]; // 기존 배열을 복사
    const prevCartItem = state.items[index]; // 위에서 찾은 인덱스로 요소를 하나만 지목.
    ...
    if (index === -1) {
      ...
      //prevCartItem.amount++; ->바깥 화면에서는 상품이 꼭 하나만 올라가는건 아니다.
